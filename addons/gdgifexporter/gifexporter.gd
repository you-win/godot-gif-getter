...
func write_frame_buffer_from_conv_image(converted_image: ConvertedImage,
	...
	result += graphic_control_extension.to_bytes()
	result += image_descriptor.to_bytes()
	result += local_color_table.to_bytes()
	result += table_image_data_block.to_bytes()
...
func export_file_data_threaded(p_data: PoolByteArray) -> PoolByteArray:
	return data + p_data + PoolByteArray([0x3b])
